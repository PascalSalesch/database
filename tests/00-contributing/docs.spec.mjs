...
describe('README.md', () => {
  ...
  it('should exist', () => { assert.strict.equal(fs.existsSync(readme), true) })
  it('should be a file', () => { assert.strict.equal(fs.statSync(readme).isFile(), true) })
  it('should be a markdown file', () => { assert.strict.equal(path.extname(readme), '.md') })
  it('should contain the package name', () => { assert.strict.equal(fs.readFileSync(readme, { encoding: 'utf-8' }).includes(packageJson.name), true) })
})
